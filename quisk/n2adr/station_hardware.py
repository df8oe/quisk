...
class FilterBoxV2:	# Control my 2016 high/low pass filter box
  ...
  def __init__(self, app, conf):
    ...
    self.old_tx_freq = 0
    self.timer = 0
    self.socket = None #socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    #self.socket.setblocking(0)
    #self.socket.connect(self.address)
  def ChangeBand(self, band):
    btn = self.application.BtnRfGain
...
class StationControlGUI(wx.Frame):    # Display a control window for my antenna tuner
  def __init__(self, hware, app, conf):
    wx.Frame.__init__(self, parent=None, title="Station Control",
    ...
    b = wx.lib.buttons.GenButton(self, -1, "HiLo")
    b.SetUseFocusIndicator(False)
    b.SetBezelWidth(4)
    b.SetFont(font)
    ...
    szr.Add(b)
    szr.AddSpacer(10)
    b = wx.lib.buttons.GenToggleButton(self, -1, "Disable Tuner Freq Control")
    b.SetFont(font)
    b.SetUseFocusIndicator(False)
    b.SetBezelWidth(4)
    self.Bind(wx.EVT_BUTTON, self.OnBtnDisable, b)
    szr.Add(b)
    szr.AddSpacer(10)
  ...
  def Open(self):	# Initialize the hardware
    ...
    self.anttuner.open()
    self.v2filter.SetTxFreq(1900000)	# Start 160 meters
  def OnBtnBand(self, event):
    ...
    f = (f1 + f2) / 2
    #self.hware.lpfilter.SetTxFreq(f)
    #self.hware.anttuner.ChangeBand(band)
    #self.hware.hpfilter.ChangeBand(band)
    self.v2filter.SetTxFreq(f)
  ...
  def OnBtnV2Preamp(self, event):
    self.v2filter.SetPreamp(event.GetEventObject().GetValue())
  def OnBtnKey(self, event):
    self.controlbox.SetKeyDown(event.GetEventObject().GetValue())
  def WriteFile(self):
    anttuner = self.anttuner
...
class App(wx.App):
  def OnInit(self):
    ...
    self.main_frame = frame = StationControlGUI (hardware, self, conf)
    frame.anttuner = AntennaTuner(self, conf)	# Control the antenna tuner
    frame.v2filter = FilterBoxV2(self, conf)	# Control V2 filter box
    #frame.lpfilter = LowPassFilter(self, conf)	# Control LP filter box
    #frame.hpfilter = HighPassFilter(self, conf)	# Control HP filter box
    frame.controlbox = ControlBox(self, conf)	# Control my Station Control Box
    frame.Open()
    frame.Show()
    return True
